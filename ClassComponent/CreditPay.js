...
validatePayment(){
  ...
  let OK =Number(this.state.CE_FormCreditAmount)+Number(this.state.CE_FormCardOwner)+Number(this.state.CE_FormCardNumber)+Number(this.state.CE_FormCVV)
  {FormCreditAmount <25 ||FormCreditAmount===""?(this.setState({CE_FormCreditAmount:0})):this.setState({CE_FormCreditAmount:1})}
  {FormCardOwner===""?(this.setState({CE_FormCardOwner:0})):this.setState({CE_FormCardOwner:1})}
  {FormCardNumber === 0 ||this.state.FormCardNumber.length<10?(this.setState({CE_FormCardNumber:0})):this.setState({CE_FormCardNumber:1})}
  {this.state.FormCVV.length<3?(this.setState({CE_FormCVV:0})):this.setState({CE_FormCVV:1})}
  if(OK<4 ){
    alert("Cannot approve Payments under 25 NIS ")
  }
  else{
 ...
 async componentDidMount(){
  ...
  render() {
    let UserCredit= this.state.UserCreditOBJ.map((UserCredit,key)=>{ return(<View key={key}>
    ...
    return (
      ...
      <View style={styles.container2} >
        <View style={{flex:1,borderWidth:2,backgroundColor:'lightblue' , textAlign:'right',direction:'rtl',maxHeight:100 ,marginBottom:15,marginTop:15,borderRadius:20}}>
          {UserCredit}
         </View>
         {/*  */}
